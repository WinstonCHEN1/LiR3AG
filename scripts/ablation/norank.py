...
def generate_reasoning_steps(question, context_list, model, client, api_url="http://localhost:11434/api/generate"):
    ...
    prompt_template = '''
You are a helpful reasoning assistant.  Given a **Question** and several **Contexts**,

* If one or more contexts provide facts you need, you can keep them.
* If context is not useful, please discard them (even if all the context is useless).
* For the preserved contexts, do not do anything with them, keep the original text.
...
{indexed_contexts}
Question: {question}
Please input the context related to the question, and **remember to keep the original text**.
'''
...
def main():
    ...
    filename = f"norank_{dataset}_{method}_{safe_name}_{timestamp}.jsonl"
