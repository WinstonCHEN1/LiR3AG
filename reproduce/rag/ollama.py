...
def call_ollama(question, context, model, api_url="http://localhost:11434/api/generate"):
    prompt = (
...
def main():
    ...
    for item in tqdm(questions, desc="Processing"):
        ...
        try:
            ...
            answer = call_ollama(question, context, args.model)

            append_jsonl(output_path, {"id": qid, "generate": answer})

            input_texts.append(full_input)
            output_texts.append(answer)
        except Exception as e:
            print(f"[Skip] ID {qid}：{e}")
            append_jsonl(output_path, {"id": qid, "generate": ""})
...
if __name__ == "__main__":
    main()
# python ollama.py --input "../../query/HotpotQA.jsonl" --output "../data" --index_dir "../../index/HotpotQA" --model llama3.1:8b --top_k 5